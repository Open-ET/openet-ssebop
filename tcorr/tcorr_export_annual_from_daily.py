#--------------------------------
# Name:         tcorr_export_annual_image.py
# Purpose:      Compute/Export annual Tcorr images
#--------------------------------
    ...
    for cycle_day, ref_date in cycle_dates.items():
        ...
        export_id = ini['EXPORT']['export_id_fmt'] \
            .format(
                ...
                date='annual_cycle{:02d}'.format(cycle_day),
                export=ini['EXPORT']['export_dest'].lower(),
        )
        logging.info('  Export ID: {}'.format(export_id))

        if ini['EXPORT']['export_dest'] == 'ASSET':
            asset_id = '{}/annual_{}'.format(
                tcorr_annual_coll_id, 'cycle{:02d}'.format(cycle_day))
